...
const BandeDeCommande = () => {
  ...
  const fetchCommande = async () => {
    ...
    setModifiedProduits(res.data.produits.map(p => ({
      id: p.id,
  ...
  const saveModifications = async () => {
    const token = localStorage.getItem("token");
    const lignesCommande = modifiedProduits.map(p => ({ id: p.id, quantite: p.quantite }));
    await axios.patch(`http://localhost:4000/commandes/${id}`, { lignesCommande }, {
      headers: { Authorization: `Bearer ${token}` },
    });
    setIsEditing(false);
    await fetchCommande();
  ...
  const handleValider = async () => {
    try {
      ...
      await axios.patch(`http://localhost:4000/commandes/valider/${id}`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="p-6 bg-white rounded shadow space-y-6">
      <h2 className="text-2xl font-bold">Bon de Commande - {commande.numeroCommande}</h2>
      <div>
...
<p><strong>Total TTC :</strong> {Number(commande.prixTotalTTC).toFixed(2)} TND</p>
      ...
      <div className="flex justify-between items-center">
        <div className="space-y-2 text-right">
          <p><strong>Total HT :</strong> {commande.prixHorsTaxe.toFixed(2)} TND</p>
          <p><strong>Total TTC :</strong> {commande.prixTotalTTC.toFixed(2)} TND</p>
        </div>
        <div className="space-x-4">
          {!isEditing ? (
            <button onClick={() => setIsEditing(true)} className="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded">Modifier</button>
          ) : (
            <button onClick={handleValider} className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-6 rounded">Valider</button>
          )}
          <button onClick={generatePDF} className="bg-gray-700 hover:bg-gray-800 text-white font-bold py-2 px-4 rounded">Télécharger PDF</button>
        </div>
      </div>
    </div>
  );
